...
//				BuildingAssignment toRemove = null;
//				for (BuildingAssignment assignment : buildingAssignments) {
//					if (unit.getType().equals(assignment.request.unit)) {
//						toRemove = assignment;
//						break;
//					}
//				}
//				if (toRemove != null) {
//					buildingAssignments.remove(toRemove);
//				}
			...
			public void onUnitComplete(Unit unit) {
				...
				/*
				 * for (BuildRequest request : constructingRequests) { if
				 * (request.getUnit().equals(unit.getType())) { if
				 * (request.getRequesterUnits() != null) {
				 * request.getRequesterUnits().add(unit); }
				 * constructingRequests.remove(unit.getType()); } }
				 */}
			...
			public void onFrame() {
				...
				// StringBuilder units = new StringBuilder("My build queue:\n");
				// checkConsistency();
				
				StringBuilder m = new StringBuilder("Started Units\n");
				for (BuildRequest req : startedRequests) {
					m.append(req.getUnit() + "\n");
				}
				game.drawTextScreen(350, 10, m.toString());
				...
				StringBuilder message = new StringBuilder("My Build Queue:\n");
				for (BuildRequest req : pendingRequests) {
					message.append(req.getUnit() + "\n");
				}
				game.drawTextScreen(10, 25, message.toString());
			}
		});
	...
	// This is O(n^2), but should be dealing with relatively few items
/*	private void checkConsistency() {
		List<UnitType> underConstruction = new ArrayList<UnitType>();
		List<BuildRequest> requests = new ArrayList<BuildRequest>(
				constructingRequests);
		for (Unit unit : self.getUnits()) {
			if (unit.isBeingConstructed() || !unit.isCompleted()) {
				underConstruction.add(unit.getType());
			}
		}
		StringBuilder message = new StringBuilder("Units Under Construction:\n");
		for (UnitType type : underConstruction) {
			message.append(type + "\n");
			for (Iterator<BuildRequest> it = requests.iterator(); it.hasNext();) {
				BuildRequest req = it.next();
				if (req.getUnit().equals(type)) {
					it.remove();
				}
			}
		}
		game.drawTextScreen(350, 10, message.toString());
		for (BuildRequest req : requests) {
			System.out.println("Resubmitting request for " + req.getUnit());
			pendingRequests.addFirst(req);
			constructingRequests.remove(req);
		}
	}
*/
	/*private void build() {
		...
		for (BuildingAssignment assignment : buildingAssignments) {
			message.append(assignment.request.unit + "\n");
			Unit worker = assignment.builder;
			BuildRequest request = assignment.request;
			...
			if (buildTile != null) {
				...
				// buildTile);
				BuildRequest added = pendingRequests.poll();
				if (worker.build(buildTile, request.getUnit())) {
					System.out.println("Success: " + added.getUnit());
					constructingRequests.add(added);
					return;
				} else {
					System.out.println("Fail: " + added.getUnit());
					pendingRequests.addFirst(added);
				}
			}
		}
		game.drawTextScreen(350, 10, message.toString());
	...
	private List<BuildRequest> constructingRequests = new ArrayList<BuildRequest>();
	private List<BuildRequest> startedRequests = new ArrayList<BuildRequest>();
//	private List<BuildingAssignment> buildingAssignments = new ArrayList<BuildingAssignment>();
...
/*		
		buildingAssignments.add(new BuildingAssignment(pendingRequests.poll(),
				worker));
...
//*/
	...
	public class BuildingAssignment {
		...
		public BuildingAssignment(BuildRequest request, Unit builder) {
